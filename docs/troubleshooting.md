...
**Diagnóstico**: `alembic heads` mostró dos cabezas — la nueva (`cd15063a2c75`) y otra, `8afb2f1757a7` ("añadir push_token a users"), que **no existía en el repositorio** (no salió en el `git pull` ni aparecía en `alembic/versions/` del checkout local). Sí existía, en cambio, en el `alembic/versions/` de la propia Pi, y la tabla `alembic_version` de Postgres ya la tenía marcada como aplicada. Conclusión: en algún momento de una sesión anterior (implementando notificaciones push) alguien ejecutó `alembic revision --autogenerate` y `alembic upgrade head` directamente en la Pi, pero el archivo `.py` generado nunca se añadió a git ni se subió — el modelo `User.push_token` sí quedó comiteado (vive en `app/models/user.py`), pero la migración que crea esa columna no. Mismo tipo de fallo que el #15 (`versions/` sin trackear), con una variante distinta: aquí el archivo sí se creó, solo que no se comiteó nunca.

**Solución**: recuperar el contenido exacto del archivo desde la Pi (`cat alembic/versions/8afb2f1757a7_...py`), añadirlo al repositorio tal cual, y reencadenar la migración nueva para que apunte a esta (`down_revision = '8afb2f1757a7'` en vez de `'32dd032060ba'`), devolviendo la cadena a una sola cabeza lineal.

**Aprendizaje**: `alembic revision --autogenerate` genera un archivo que hay que comitear explícitamente, igual que cualquier otro cambio de código — no es un artefacto de build que se pueda regenerar solo. La regla ya estaba escrita en `backend/README.md` ("los archivos de migración se generan localmente y hay que comitearlos"), pero "localmente" en la práctica había significado "en la Pi" para esa migración concreta, no en el checkout de desarrollo desde el que se hacen los commits — vale la pena, tras cualquier `alembic revision --autogenerate` ejecutado directamente en la Pi (en vez de en local), copiar el archivo resultante de vuelta al checkout de desarrollo y comitearlo ahí antes de darlo por terminado.

## 21. Aviso push de "desconectado" con el dispositivo funcionando perfectamente al mirar

**Síntoma**: llega una notificación "macetero01 se ha desconectado inesperadamente", pero al abrir la app o el dashboard segundos después todo está normal — el dispositivo online, datos llegando.

**Diagnóstico**: el firmware nunca fija un `setKeepAlive()` en el cliente MQTT (`PubSubClient`), así que usa el valor por defecto de la librería, 15 segundos. Por especificación MQTT, el broker (Mosquitto) da por muerta una conexión y dispara su LWT (`{"online":false}`, retenido) a partir de 1.5× ese keepalive, es decir, ~22 segundos de silencio. Un simple parpadeo de wifi doméstico (cambio de canal del router, renovación de la concesión DHCP, una interferencia puntual) es más que suficiente para superar esa ventana sin que el ESP32 llegue a darse cuenta — su `loop()` solo reacciona a que `client.connected()` se vuelva falso, y `reconnectMQTT()` ya lo resuelve solo en unos segundos en cuanto el wifi vuelve. El problema real no era el wifi, sino que **ni Node-RED ni el backend esperaban nada antes de reaccionar**: el backend (`backend/app/services/mqtt_client.py`, `_on_message`) enviaba el push en el mismísimo mensaje `online:false`, sin comprobar si seguía así unos segundos después, y tampoco había ninguna notificación de "ha vuelto" que pudiera servir de contraste.
...
## 28. El diagnóstico visual nunca recibía los datos de los sensores, solo la foto

**Síntoma**: ninguno visible a nivel de error — la primera petición real contra Groq devolvió `200 OK` con un veredicto y un mensaje coherentes ("bien", hablando de flores y hojas), así que a primera vista parecía funcionar. Luis preguntó explícitamente si el modelo tenía en cuenta algo más aparte de la foto, porque el mensaje no mencionaba nada de humedad pese a que ese mismo dispositivo llevaba días en exceso de agua según los sensores (confirmado en varias entradas anteriores de este mismo documento).

**Diagnóstico**: al reescribir `plant_vision.py` para pasar de Gemini a Groq (entrada #27), la función `_resumen_sensores()` se seguía llamando y su resultado se guardaba en la variable `contexto` — pero la línea que añadía ese texto al mensaje enviado al modelo (`instrucciones += f"\nDatos de los sensores:\n{contexto}"`) se perdió en la reescritura. El modelo nunca vio ni una sola cifra de humedad o temperatura; solo la foto. La API respondía con éxito y un texto plausible, lo que ocultaba por completo que faltaba la mitad de la información prometida.
