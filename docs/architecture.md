...
Igual que los umbrales de riego, la calibración del sensor capacitivo de humedad de suelo (dos valores de referencia: lectura ADC en seco y en húmedo) dejó de estar fija en firmware para pasar a ser configuración editable — mismo principio de "el servidor decide y guarda, el ESP32 solo aplica" que el resto de la configuración persistente.

Columnas `devices.soil_dry_raw`/`soil_wet_raw` en Postgres, con los mismos valores que ya llevaba el firmware a mano como `server_default` (así añadir las columnas no cambia nada hasta que alguien recalibra de verdad). Se capturan desde la app (`POST /api/devices/{id}/calibrate`, body `{"punto": "seco" | "humedo"}`): el endpoint lee el valor crudo más reciente publicado por el dispositivo (`humedad_suelo_raw`, ver el modelo de datos de InfluxDB más abajo) con una consulta puntual sin agregar — `aggregateWindow()` sirve para gráficas e histórico, no para "qué marca el sensor ahora mismo" — lo guarda, y empuja la nueva calibración al dispositivo por el mismo topic MQTT retained que ya llevaba `humedadMin`/`humedadMax` (`maceteros/{id}/config`). El ESP32 la aplica en `soilMoisturePercent()` sin decidir nada por su cuenta.
...
**Proveedor y modelo, en concreto**: [Groq](https://groq.com/) (`app/services/plant_vision.py`), un proveedor de inferencia que aloja modelos abiertos de terceros (no entrena sus propios modelos, sino que los sirve muy rápido sobre hardware propio, LPUs en vez de GPUs). El modelo usado es `qwen/qwen3.8-27b` — Qwen es la familia de modelos abiertos de Alibaba Cloud; esta es su variante de 27B de parámetros con soporte de visión (recibe imagen + texto), la única con visión que Groq ofrece en este momento. Plan gratuito real de Groq para este modelo (documentado, no un trial que caduca, sin tarjeta): **30 peticiones/minuto, 1.000 peticiones/día, 8.000 tokens/minuto, 200.000 tokens/día** — de sobra para un uso a demanda de una persona (unas pocas fotos por semana), muy por debajo del límite incluso en pruebas intensivas.

**Solo se guarda la última foto** (`plant_photo_diagnoses`, una fila por dispositivo, se sobreescribe) — pero antes de sobreescribirla se usa: si ya había una foto anterior, se le manda al modelo junto con la nueva para que compare evolución (mejor/igual/peor), no solo el estado puntual. La foto vive como `bytea` en Postgres, no en el filesystem de la Pi — es el primer y único blob binario del proyecto, y no justificaba inventar una convención de almacenamiento en disco nueva para un solo fichero pequeño que además se sobreescribe.

Es el primer punto del proyecto donde contenido real (la foto) sale hacia un servicio de IA de terceros — misma clase de decisión que Open-Meteo (API externa aceptada cuando aporta algo que no se puede hacer local).

**Historial real de proveedor, tres intentos antes de asentarse**: primero Claude (API de pago de Anthropic) — funcionaba bien, pero implicaba un gasto (céntimos al mes) que Luis prefería evitar para este uso a demanda. Se cambió a Gemini buscando coste cero, pero su nivel "gratuito" resultó estar topado de verdad para la cuenta usada (Google AI Studio mostró un aviso de "Upgrade to unlock more" tras varias pruebas — no era un pico de demanda puntual como parecía al principio con los primeros 503). La solución final es Groq: tiene un plan gratuito real y publicado (límites por día/minuto documentados, no un trial que caduca), sin tarjeta, en una cuenta distinta de Google que no arrastra el uso previo del intento anterior. Lección para cualquier integración futura con un "nivel gratuito" de terceros: confirmarlo probándolo de verdad con la cuenta real que se va a usar, no fiarse solo de lo que dice la documentación — el mismo "gratis" significa cosas distintas según el proveedor y según el historial de la cuenta concreta.

**Deliberadamente no autoajusta nada.** Se consideró ir más lejos y que el propio sistema ajustara `humedad_min`/`humedad_max`/duración de riego a partir del veredicto — descartado por dos motivos: con una sola planta y fotos a demanda, no hay ni de lejos histórico suficiente para que un ajuste automático tenga base real (mismo criterio que el punto 4 del roadmap, más abajo), y automatizar un cambio sobre un actuador físico (la bomba) sin que una persona lo confirme es el único punto del proyecto que, si se equivoca, hace daño real a la planta sin que nadie se entere a tiempo. Si en el futuro se construye, sería como sugerencia que el usuario aprueba, nunca aplicada sola.
...
**2. Interior / exterior como atributo del dispositivo — construido.** Columna `environment` en `devices` (`"interior"` / `"exterior"` / sin especificar), editable desde la misma pantalla de configuración que el tipo de planta. Deliberadamente no toca el ciclo de riego ni Node-RED en esta pieza — es solo la bandera que el punto 3 (meteorología) todavía no consulta. Las plantas de interior no la necesitan y no deberían pagar ese coste (una llamada a una API externa) en cada ciclo de decisión, así que el valor por defecto es "sin especificar", no "exterior".

**3. Integración con una API meteorológica para exteriores — construido.** [Open-Meteo](https://open-meteo.com/), sin clave de API ni tarjeta (`backend/app/services/weather_client.py`). Reutiliza el mismo mecanismo de sondeo del punto 1: el backend calcula `lluviaPrevista` por dispositivo (`true` solo si `environment == "exterior"` y su `Location` tiene coordenadas, con un umbral de 50% de probabilidad en las próximas 6 horas y una caché en memoria de 45 minutos para no golpear el API en cada sondeo de 60s de Node-RED) y lo añade al mismo objeto que ya viaja por `GET /api/internal/device-configs`. **"Decisión riego" no sabe nada de `environment` ni de coordenadas** — solo gana una condición más (`&& sinLluviaPrevista`) sobre un booleano que ya le llega hecho. Diseñado a prueba de fallos: cualquier error consultando Open-Meteo (timeout, red caída, respuesta inesperada) se trata exactamente igual que "sin lluvia prevista" — el sistema nunca deja de regar por humedad baja a causa de un fallo ajeno de una API externa. Las coordenadas se guardan en `Location.latitude`/`longitude` (nuevas columnas opcionales), editables desde la app en la pantalla de cada ubicación.

**4. Aprendizaje automático por tipo de planta.** Un primer paso relacionado, pero distinto, ya está construido: el resumen periódico de "salud de la planta" (`backend/app/services/health_analysis.py`) calcula un ritmo de secado real (%/hora) y lo compara contra un valor esperado por tipo de planta — pero ese valor esperado se derivó investigando guías de cuidado de plantas reales, no ajustando sobre histórico propio, así que es un punto de partida razonado, no la pieza de ML en sí. La pieza más ambiciosa y la que más depende de las otras tres: ajustar con el tiempo los umbrales del catálogo del punto 1 a partir de los datos históricos reales de InfluxDB (humedad, temperatura, riegos ejecutados) agrupados por tipo de planta, en vez de dejarlos fijos para siempre. Deliberadamente la última en construirse, por dos motivos: necesita que ya exista el campo "tipo de planta" del punto 1 para poder agrupar datos por planta, y necesita semanas o meses de histórico acumulado por tipo antes de que cualquier ajuste automático tenga sentido estadístico — construirla antes de tener datos suficientes sería optimizar sobre ruido. La primera versión razonable no es necesariamente un modelo entrenado: empezar por un ajuste estadístico simple (medias/percentiles observados por tipo de planta) y solo subir a un modelo más sofisticado si ese primer paso se queda corto, es el mismo criterio de "la solución más simple que resuelve el problema real" que ya ha guiado el resto del proyecto (ver por ejemplo la elección de PostgreSQL frente a alternativas más complejas, arriba).
...
- El riego se dosifica por **tiempo**, no por volumen. Un caudalímetro permitiría dosificación volumétrica real y detectar la degradación de la bomba (menos caudal para el mismo tiempo de funcionamiento) — en estudio de compatibilidad con la bomba actual, no construido todavía.
