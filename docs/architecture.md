...
**URL del servidor configurable en tiempo de ejecución, no fija en el código**: se guarda en `SecureStore` y se edita desde la app. Es lo que permite usar la misma build tanto en la LAN de casa como con una URL de Cloudflare Tunnel sin recompilar — la pieza que evita duplicar trabajo entre "desarrollar en casa" y "usar la app fuera".
