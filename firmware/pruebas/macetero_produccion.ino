...
// ---------- MQTT ----------
const char* mqtt_server = "192.168.100.52";
...
#define HORA_INICIO_RIEGO 8
#define HORA_FIN_RIEGO 21
...
void loop() {
  ...
  if (millis() - lastSend > 4000) {
    ...
    // ----------- CONTROL DE RIEGO (se moverá a la plataforma más adelante) -----------
    bool esHorarioPermitido = (now.hour() >= HORA_INICIO_RIEGO && now.hour() < HORA_FIN_RIEGO);
    bool necesitaRiego = (soilPct < 36);
    bool hanPasado24h = (now.unixtime() - ultimoRiego >= 86400);

    if (necesitaRiego && esHorarioPermitido && hanPasado24h) {
