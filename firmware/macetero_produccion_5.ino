...
void reconnectMQTT() {
  while (!client.connected()) {
    ...
    // El último parámetro (payload del LWT) lo publica el broker
    // automáticamente si este cliente se desconecta sin avisar.
    // retained=true para que un suscriptor que llegue después lo vea.
    ...
    if (conectado) {
      ...
      // Anunciamos que estamos vivos (retained, sustituye al LWT anterior)
...
void loop() {
  ...
  // ---- Arranque del riego ----
  // Se ignora una orden nueva si ya hay un riego en curso.
  ...
  // ---- Parada del riego ----
  // Se evalúa en cada vuelta del loop, sin bloquear.
  // El tope de seguridad garantiza que la bomba se apague aunque
  // TIEMPO_RIEGO se configurase con un valor erróneo.
